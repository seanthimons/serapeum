...
Purpose: These functions are the foundation for all store lifecycle operations. The UI layer (Plan 02) will call check_store_integrity, rebuild_notebook_store, and find_orphaned_stores. The ensure_ragnar_store function is designed to be called by Phase 22 (Module Migration) when embedding code paths are switched to per-notebook stores — it is NOT wired into modules in this phase.
