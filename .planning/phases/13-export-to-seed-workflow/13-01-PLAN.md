...
The current `output$detail_actions` renderUI only shows a close button. Replace it with logic that also shows a "Use as Seed" button when the paper has a DOI. Exact code pattern:

```r
output$detail_actions <- renderUI({
  if (is.null(viewed_paper())) return(NULL)

  # Look up the paper to check for DOI (same pattern as abstract_detail at line ~691)
  papers <- papers_data()
...
This replaces the existing `output$detail_actions` block (lines ~858-866).
...
- Add an `observeEvent(input$use_as_seed, { ... })` handler that:
  ...
  - Sets `seed_request(list(doi = paper$doi, ts = Sys.time()))` — the timestamp ensures each click produces a unique value so `observeEvent` in app.R always fires, even for the same DOI clicked twice
...
4. **Add observeEvent to wire the bridge** after the search notebook server call (after the new line ~859), before the existing `observeEvent(discovery_request(), ...)` block:
   ...
   **Re-trigger prevention:** No reset of `search_seed_request` is needed. Because Task 1 sets `seed_request(list(doi = ..., ts = Sys.time()))`, every click produces a unique list value (different timestamp). `observeEvent` compares by value and will always fire on a new click. Even clicking the same paper twice works because `Sys.time()` differs. There is no need to reset the reactive to NULL — the timestamp approach eliminates the re-trigger problem entirely.
