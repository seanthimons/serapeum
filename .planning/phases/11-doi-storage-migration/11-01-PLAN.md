...
must_haves:
  ...
  artifacts:
    ...
    - path: "R/utils_doi.R"
      ...
      exports: ["normalize_doi", "is_valid_doi", "generate_citation_key"]
    - path: "R/db.R"
  ...
  key_links:
    ...
    - from: "R/db.R::create_abstract"
      to: "R/utils_doi.R::normalize_doi"
      via: "DOI normalized before INSERT"
      pattern: "normalize_doi"
...
2. Create `R/utils_doi.R` with three exported functions:
   - `normalize_doi(doi)`: Strip URL prefixes (https://doi.org/, https://dx.doi.org/, doi: prefix), trim whitespace, lowercase, validate starts with "10." — return bare DOI or NA_character_ if invalid/NULL/NA/empty. NOTE: This is for STORAGE normalization (bare DOI). The existing `normalize_doi()` in api_openalex.R is for API lookup (returns full URL). Name this one `normalize_doi_for_storage()` to avoid collision, OR since the existing one is in api_openalex.R namespace context, just name it `normalize_doi` in utils_doi.R — but VERIFY there is no naming conflict at source() time. If conflict, use `normalize_doi_bare()`.
...
3. Update `create_abstract()` in `R/db.R` (around line 536):
   ...
   - Add DOI value conversion (normalize using the new normalize function from utils_doi.R): `doi_val <- if (is.null(doi) || is.na(doi) || doi == "") NA_character_ else normalize_doi(doi)` — use whichever function name avoids collision
   - Add `doi` to the INSERT column list and VALUES placeholders (18th column/parameter)
   - Add `doi_val` to the parameter list

IMPORTANT: The existing `normalize_doi()` in `R/api_openalex.R` (line 417) returns a full URL format for API lookups. The new storage normalizer must return BARE DOI (just "10.xxxx/yyyy"). Check at source time if both files are sourced — if so, use a distinct name like `normalize_doi_bare()` in utils_doi.R.
  </action>
  <verify>
    - File `migrations/005_add_doi_column.sql` exists with ALTER TABLE and CREATE INDEX statements
    - File `R/utils_doi.R` exists with normalize, validate, and citation key functions
    - `create_abstract()` in R/db.R accepts `doi` parameter and includes it in INSERT
  ...
  <done>
    ...
    - DOI normalization strips URL prefix, lowercases, validates "10." prefix
...
2. Add `backfill_dois()` function to `R/db.R`:
   ...
   - For each returned work, extract DOI (strip https://doi.org/ prefix), normalize with the storage normalizer, UPDATE abstracts SET doi = ? WHERE id = ?
...
4. Source `R/utils_doi.R` — verify it gets sourced. Check if app.R or a global.R sources R/ files. If using `source()` calls, add `source("R/utils_doi.R")`. If Shiny auto-sources R/ directory, no action needed (Shiny sources all R/*.R files alphabetically).
  ...
  <verify>
    ...
    - `backfill_dois()` function exists in db.R
  ...
  <done>
    ...
    - backfill_dois() can fetch DOIs for existing papers in batches of 50
...
- No naming conflicts between normalize functions
