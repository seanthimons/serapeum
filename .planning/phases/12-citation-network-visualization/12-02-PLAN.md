...
must_haves:
  ...
  key_links:
    ...
    - from: "R/mod_search_notebook.R"
      to: "app.R"
      via: "Network icon on result cards triggers view switch to citation network"
      pattern: "network|citation"
---
...
5. **Seeded paper search integration** (entry from existing search results — REQUIRED, locked user decision):
   - In R/mod_search_notebook.R, add a "Network" icon button (icon("project-diagram")) on each result card in the abstract detail view. When clicked, sends the paper_id to the network module via a shared reactiveVal or session$sendCustomMessage
   - Add an "Explore Citations" button in the abstract detail panel (alongside existing action buttons). When clicked, same behavior: sends paper_id to network module
   - Wire click events: set current_view("network") and current_network() with the paper's OpenAlex ID, triggering the network module to build a fresh graph with that paper as seed
   - Both entry points (sidebar "New Network" AND from search results) MUST work
