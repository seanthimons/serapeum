...
must_haves:
  ...
  artifacts:
    - path: "R/api_openalex.R"
      provides: "Fixed OpenAlex auth (no Bearer header) + friendly error wrapping"
      contains: "req_url_query.*api_key"
    - path: "R/api_openrouter.R"
      provides: "Friendly error wrapping for OpenRouter API calls"
      contains: "classify_api_error"
  ...
  key_links:
    ...
    - from: "R/api_openalex.R"
      ...
      via: "tryCatch wrapping with classified errors"
      pattern: "tryCatch.*search_papers"
    - from: "R/mod_search_notebook.R"
      to: "UI toast notifications"
      via: "showNotification with severity styling"
...
<task type="auto">
  <name>Task 1: Fix OpenAlex 401 auth and create error classification helper</name>
...
**Error classification helper (for BUGF-02):**
Add a helper function `classify_api_error()` at the top of `R/api_openalex.R` (it will be sourced and available globally since all R/*.R files are sourced):
...
- In `search_papers()` (line 251-255): Replace `stop("OpenAlex API error: ", e$message)` with a classified error. Re-throw with the classified message so callers get a structured error:
  ```r
  resp <- tryCatch({
    req_perform(req)
  }, error = function(e) {
  ...
  ```
- In `fetch_all_topics()` (line 621-625): Same pattern — replace `stop("OpenAlex API error while fetching topics: ", e$message)` with classified error.
...
- In `chat_completion()` (line 45-49): Replace `stop("OpenRouter API error: ", e$message)` with classified error using `classify_api_error(e, "OpenRouter")`.
...
- Do NOT touch `list_models()`, `list_embedding_models()`, `list_chat_models()`, `get_openrouter_credits()` — these already have graceful fallbacks returning defaults/NULL.
  ...
  3. Confirm `search_papers()`, `fetch_all_topics()`, `chat_completion()`, `get_embeddings()` all use classified errors.
  4. Run: `"C:\Program Files\R\R-4.5.1\bin\Rscript.exe" -e "source('R/api_openalex.R'); source('R/api_openrouter.R'); cat('Sourced OK\n')"` — no syntax errors.
  </verify>
  <done>
  ...
  - All main API functions use classified errors instead of raw error messages
  </done>
</task>
...
**Create a toast helper** at the top of `R/mod_search_notebook.R` (will be sourced globally):
...
- Chat error handler (line 1640-1644): The `rag_query` call catches errors and returns string. Update to use classified error:
  ```r
  response <- tryCatch({
...
**Update `app.R`:**
- Topic request error handler (around line 885-887): Replace `showNotification(paste("Search error:", e$message), type = "error")` with classified error toast.
- Discovery request error handlers: Check if there are tryCatch blocks for `get_citing_papers`, `get_cited_papers`, `get_related_papers` calls. These are in the `withProgress` block (lines 712-717) but don't have explicit error handling — the functions themselves return NULL/empty on error. Add a tryCatch around the switch statement:
  ...
  <verify>
  1. Search for all `showNotification.*error` calls in the modified files — each should use `show_error_toast()` or `classify_api_error()` for API-related errors (non-API errors like "Search query cannot be empty" should keep simple `showNotification`).
  2. Confirm `show_error_toast()` function exists with `message`, `details`, `severity` params.
  3. Run: `"C:\Program Files\R\R-4.5.1\bin\Rscript.exe" -e "library(shiny); library(htmltools); for(f in list.files('R', pattern='\\.R$', full.names=TRUE)) source(f); cat('All sourced OK\n')"` — no syntax errors.
  </verify>
  <done>
...
3. Confirm `classify_api_error()` and `show_error_toast()` exist
4. Confirm no combined `observeEvent(list(input$refresh_search, ...))` pattern remains
5. Run tests: `"C:\Program Files\R\R-4.5.1\bin\Rscript.exe" -e "testthat::test_dir('tests/testthat')"`
</verification>
