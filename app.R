...
if (!dir.create(ragnar_dir, showWarnings = FALSE, recursive = TRUE)) {
  if (!dir.exists(ragnar_dir)) {
    stop(
      "Failed to create ",
      ragnar_dir,
      " directory. Check permissions and disk space."
    )
  }
}
    ...
    div(
      ...
      div(
        ...
        div(
          ...
          actionLink(
            "cost_link",
            label = tagList(icon_dollar(), "Details"),
      ...
      div(
        ...
        actionLink(
          "settings_link",
          label = tagList(icon_settings(), "Settings"),
          class = "text-muted small"
        ),
        actionLink(
          "about_link",
          label = tagList(icon_circle_info(), "About"),
          class = "text-muted small"
        )
      )
    ),
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$dark_mode,
    {
      session$sendCustomMessage(
        ...
        message = list(
          theme = if (input$dark_mode == "dark") "dark" else "light"
        )
      )
    },
    ignoreNULL = FALSE,
  ...
  observe({
    if (!is.null(config_file)) {
      has_openrouter <- !is.null(get_setting(
        config_file,
        "openrouter",
        "api_key"
      )) &&
      ...
      if (has_openrouter || has_openalex) {
        ...
        if (has_openrouter) {
          msg <- paste(msg, "OpenRouter API key")
        }
        if (has_openrouter && has_openalex) {
          msg <- paste(msg, "+")
        }
        if (has_openalex) {
          msg <- paste(msg, "OpenAlex email")
        }
  ...
  observeEvent(
    effective_config(),
    {
      ...
      cisa <- isTRUE(get_setting(cfg, "compliance", "cisa_filter"))
      tryCatch(
        {
          # Fetch chat model pricing
          chat_models_df <- list_chat_models(api_key, cisa_filter = cisa)
          if (!is.null(chat_models_df) && nrow(chat_models_df) > 0) {
            update_model_pricing(chat_models_df[, c(
              "id",
              "prompt_price",
              "completion_price"
            )])
          ...
          embed_models_df <- list_embedding_models(api_key, cisa_filter = cisa)
          if (
            !is.null(embed_models_df) &&
              nrow(embed_models_df) > 0 &&
  ...
  output$oa_sidebar_badge <- renderUI({
    ...
    if (is.null(oa_key) || !nzchar(trimws(oa_key))) {
      return(NULL)
    }
    ...
    if (oa_toast_should_fire(con, pct)) {
      ...
      showNotification(
        sprintf(
          "OpenAlex daily budget is %d%% consumed. Resets at midnight UTC.",
          pct
        ),
        type = "warning",
        duration = 10
  ...
  observe({
    ...
    lapply(networks$id, function(net_id) {
      observeEvent(
        input[[paste0("select_network_", net_id)]],
        {
  ...
  observe({
    ...
    lapply(networks$id, function(net_id) {
      ...
      if (is.null(delete_network_observers[[net_id_str]])) {
        delete_network_observers[[net_id_str]] <- observeEvent(
          input[[paste0("delete_network_", net_id)]],
          {
  ...
  wizard_modal <- function() {
    wizard_step <- function(
      ...
      div(
        ...
        span(
          class = "badge bg-primary rounded-pill fs-6 mt-1",
        ...
        div(
          ...
          actionButton(btn_id, btn_label, class = paste("btn-sm", btn_class))
        )
      )
    }
  ...
  observe({
    papers <- network_seed_papers()
    if (is.null(papers) || length(papers) == 0) {
      return()
    }

    lapply(seq_along(papers), function(i) {
      observeEvent(
        input[[paste0("select_network_seed_", i)]],
        {
          paper <- papers[[i]]
          removeModal()
          ...
          showNotification(
            paste("Building network for:", paper$title),
            type = "message"
          )
        },
        ignoreInit = TRUE
  ...
  observeEvent(input$new_search_nb, {
    showModal(modalDialog(
      ...
      div(
        ...
        checkboxInput(
          "search_exclude_retracted",
          "Exclude retracted papers",
          value = TRUE
        ),
        checkboxInput(
          "search_flag_predatory",
          "Flag predatory journals/publishers",
          value = TRUE
        ),
        numericInput(
          "search_min_citations",
          "Minimum citations (optional)",
          value = NA,
          min = 0,
          max = 10000,
          step = 1
  ...
  output$quality_cache_status_create <- renderUI({
    ...
    div(
      class = "mt-2 small",
      div(
        class = if (status$is_empty || status$is_stale) {
          "text-warning"
        } else {
          "text-muted"
        },
  ...
  observeEvent(input$refresh_quality_cache_create, {
    ...
    if (result$success) {
      showNotification(
        sprintf(
          "Quality data ready: %d publishers, %d journals, %d retractions",
      ...
      if (!result$predatory_publishers$success) {
        failed <- c(failed, "publishers")
      }
      if (!result$predatory_journals$success) {
        failed <- c(failed, "journals")
      }
      if (!result$retraction_watch$success) {
        failed <- c(failed, "retractions")
      }
      showNotification(
      ...
      return(
        div(
          ...
          div(
            ...
            p(class = "lead text-muted", "Your AI-powered research assistant"),
            p(
              class = "text-muted small",
          ...
          landing_step(
            ...
            div(
              ...
              actionButton(
                "welcome_import",
                "Import Papers",
                class = "btn-sm btn-outline-peach",
                icon = icon_file_import()
              ),
              actionButton(
                "welcome_doc_nb",
    ...
    if (nb$type == "document") {
      tagList(
        div(
          ...
          h4(
            class = "mb-0",
            tagList(icon_file_pdf(class = "text-danger me-2"), nb$name)
          ),
          actionButton(
            "delete_nb",
            NULL,
      ...
      tagList(
        div(
          ...
          div(
            ...
            h4(
              class = "mb-0",
              tagList(icon_search(class = "text-primary me-2"), nb$name)
            ),
            actionButton(
              "delete_nb",
              NULL,
  ...
  discovery_request <- mod_seed_discovery_server(
    "seed_discovery",
    reactive(con),
    config_file_r,
    pre_fill_doi
  ...
  query_request <- mod_query_builder_server(
    "query_builder",
    reactive(con),
    config_file_r
  )
  ...
  topic_request <- mod_topic_explorer_server(
    "topic_explorer",
    reactive(con),
    config_file_r
  )
  ...
  observeEvent(
    search_nb_result$network_seed_request(),
    {
  ...
  observeEvent(
    sidebar_import_api$network_seed_request(),
    {
  ...
  observeEvent(discovery_request(), {
    ...
    withProgress(message = "Fetching related papers...", {
      papers <- tryCatch(
        {
          switch(
            ...
            cited_by = get_cited_papers(
              req$seed_paper$paper_id,
              email,
              api_key
            ),
            related_to = get_related_papers(
              req$seed_paper$paper_id,
              email,
              api_key
            )
      ...
      if (nrow(seed_existing) == 0) {
        ...
        if (
          !is.null(seed$abstract) &&
            !is.na(seed$abstract) &&
            nchar(seed$abstract) > 0
        ) {
  ...
  observeEvent(query_request(), {
    ...
    showNotification(
      paste("Created notebook with", length(results), "papers"),
      type = "message"
    )
  })
  ...
  observeEvent(input$delete_nb, {
    ...
    showModal(modalDialog(
      ...
      p(
        class = "text-danger",
        "This will permanently delete all documents and data in this notebook."
      ),
      footer = tagList(
        modalButton("Cancel"),
