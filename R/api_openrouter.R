  ...
  if (
    is.null(content) ||
      (is.character(content) &&
        (length(content) == 0 || all(nchar(content) == 0)))
  ) {
...
list_chat_models <- function(api_key, cisa_filter = FALSE) {
  ...
  df <- data.frame(
    ...
    context_length = sapply(chat_models, function(x) {
      as.integer(x$context_length %||% 0)
    }),
...
format_chat_model_choices <- function(models_df) {
  ...
  labels <- sapply(seq_len(nrow(models_df)), function(i) {
    row <- models_df[i, ]
  ...
  tryCatch(
    {
      ...
      list(
        ...
        remaining = as.numeric(data$total_credits %||% 0) -
          as.numeric(data$total_usage %||% 0)
      )
    },
