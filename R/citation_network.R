...
#'
#' @param cited_by_counts Numeric vector of metric values
...
compute_node_sizes <- function(cited_by_counts) {
  ...
  safe_values <- pmax(cited_by_counts, 0, na.rm = TRUE)
...
build_network_data <- function(nodes_df, edges_df, palette = "viridis",
  ...
  nodes_df$value <- compute_node_sizes(get_sizing_metric(nodes_df, size_by))
